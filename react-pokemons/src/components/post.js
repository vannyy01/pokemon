...
const image = 'https://pokeapi.co/media/img/';
...
function renderTypes(types) {
    return types.map((type) => {
            switch (type.name) {
                case "poison":
                    return <a key={type.name} className="btn btn-m btn-dark"
                                   href={image + type.resource_uri}>{_.capitalize(type.name)}</a>;
                case "water":
                    return <a key={type.name} className="btn btn-m btn-primary"
                                   href={image + type.resource_uri}>{_.capitalize(type.name)}</a>;
                case "bug":
                    return <a key={type.name} className="btn btn-m btn-success"
                                   href={image + type.resource_uri}>{_.capitalize(type.name)}</a>;

                case "flying":
                    return <a key={type.name} className="btn btn-m btn-info"
                                   href={image + type.resource_uri}>{_.capitalize(type.name)}</a>;
                case "normal":
                    return <a key={type.name} className="btn btn-m btn-secondary"
                                   href={image + type.resource_uri}>{_.capitalize(type.name)}</a>;
                default:
                    return false;
            }
